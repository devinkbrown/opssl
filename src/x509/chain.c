...
int opssl_x509_verify(const opssl_x509_chain_t *chain, const opssl_x509_store_t *store,
    ...
    /* 6. Check certificate revocation via CRL if CRLs are loaded */
...
/* ─── CRL (Certificate Revocation List) Implementation ──────────────── */
    ...
    if (opssl_cbs_len(&tbs_seq) > 0 &&
        ...
        if (opssl_asn1_get_sequence(&tbs_seq, &revoked_seq)) {
            while (opssl_cbs_len(&revoked_seq) > 0 &&
                ...
                opssl_cbs_t entry;
                if (!opssl_asn1_get_sequence(&revoked_seq, &entry))
                    break;

                opssl_cbs_t serial_int;
                if (!opssl_asn1_get_integer(&entry, &serial_int))
                    break;
...
{
    if (!store || !crl || store->crl_count >= OPSSL_MAX_CRLS)
        return 0;
                            ...
                            const opssl_x509_store_t *store)
{
    ...
    /* Hash cert's issuer DN for CRL matching */
    const uint8_t *cert_der;
    size_t cert_der_len;
    ...
    if (!opssl_asn1_get_integer(&tbs, &serial_skip))
        return 1;

    /* Skip signature AlgorithmIdentifier */
    opssl_cbs_t sig_skip;
    if (!opssl_asn1_get_sequence(&tbs, &sig_skip))
        return 1;

    /* Now at issuer Name */
    const uint8_t *issuer_start = opssl_cbs_data(&tbs);
    opssl_cbs_t issuer_name;
    if (!opssl_asn1_get_sequence(&tbs, &issuer_name))
        return 1;
    ...
    for (size_t c = 0; c < store->crl_count; c++) {
        ...
        if (memcmp(crl->issuer_hash, issuer_hash, 32) != 0)
            continue;

        if (crl->next_update > 0 && now > crl->next_update)
            continue;
                           ...
                           const opssl_x509_store_t *store)
{
    ...
    /* Extract BasicOCSPResponse from OCTET STRING */
    ...
    /* Skip optional version [0] in ResponseData */
    ...
    /* Skip responderID (byName [1] or byKey [2]) */
    if (opssl_cbs_len(&tbs_resp) == 0)
        return OPSSL_OCSP_UNKNOWN;
    ...
    /* Get target cert serial for matching */
    ...
    while (opssl_cbs_len(&responses) > 0) {
        ...
        if (!opssl_asn1_get_sequence(&responses, &single_resp))
            break;
         ...
         */
        opssl_cbs_t cert_id;
        if (!opssl_asn1_get_sequence(&single_resp, &cert_id))
            break;
        ...
        if (opssl_cbs_len(&single_resp) == 0)
            break;
        uint8_t status_tag = opssl_cbs_data(&single_resp)[0] & 0x1F;
        if (status_tag == 0)
            result = OPSSL_OCSP_GOOD;
        else if (status_tag == 1)
            result = OPSSL_OCSP_REVOKED;
        else
            result = OPSSL_OCSP_UNKNOWN;
        break;
    }

    /* Verify OCSP response signature against trust store */
    if (result == OPSSL_OCSP_GOOD && store) {
        bool sig_verified = false;

        /* Try embedded responder certificate first */
